...
def test_conv2d_stats_fprop():
    params = Conv2dGw8Kernel.params_cls(N=16, H=32, W=64, C=128, R=3, S=3)
    stats = Conv2dGw8Kernel.stats_cls(params, output_names="output")
...
def test_conv2d_stats_input_grad():
    params = Conv2dGw8Kernel.params_cls(N=16, H=32, W=64, C=128, R=3, S=3)
    stats = Conv2dGw8Kernel.stats_cls(params, output_names="grad_input")
    read = (16 * 32 * 64 * 128 + 128 * 8 * 3 * 3) * 2
    written = 16 * 32 * 64 * 128 * 2
    assert stats.bytes_read == read
    assert stats.bytes_written == written
...
def test_conv2d_stats_weight_grad():
    params = Conv2dGw8Kernel.params_cls(N=16, H=32, W=64, C=128, R=3, S=3)
    stats = Conv2dGw8Kernel.stats_cls(params, output_names="grad_weight")
