...
BATCH_SIZE=$2
BENCH=${3:-train}
COMPILEMODE=${4:-default}

SPIO_LOGGER=1 python3 benchmark.py --model $MODEL --amp --channels-last --bench=$BENCH   --batch-size=$BATCH_SIZE  --model-kwargs group_size=8 --spio --torchcompile --torchcompile-mode=$COMPILEMODE
SPIO_LOGGER=1 python3 benchmark.py --model $MODEL --amp --channels-last --bench=$BENCH   --batch-size=$BATCH_SIZE --torchcompile
SPIO_LOGGER=1 python3 benchmark.py --model $MODEL --amp --channels-last --bench=$BENCH   --batch-size=$BATCH_SIZE  --model-kwargs group_size=8 --torchcompile --torchcompile-mode=$COMPILEMODE
