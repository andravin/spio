...
if reflection.stackable:
    ...
    args_lst = [
        reflection.make_args(params, device=args.device) for _ in range(args.depth)
    ]

if not args.skip_kernels:
    kernel_args_lst = [reflection.arrange_kernel_args(args) for args in args_lst]
    best_result, _, results = spio.kernels.benchmark(
        kernel_class,
        params,
        kernel_args_lst,
        configs=configs,
...
if args.benchmark_torch:
    ...
    if reflection.is_grad:
        ...
        result = spio.kernels.benchmark_function(
            kernel_class,
