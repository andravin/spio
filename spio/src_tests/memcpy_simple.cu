...
extern "C" {
    __global__ void memcpy_simple(uint4* __restrict__ dst, const uint4* __restrict__ src) {
        ...
        for (int i = 0; i < ITERS; ++i) {
            if ((i + 1) < ITERS) {
                __pipeline_memcpy_async(smem_write + ping_pong * THREADS,
                                        src_load + (i + 1) * THREADS, sizeof(uint4), 0);
            }
            __pipeline_commit();
