...
template <typename T, bool is_enum = std::is_enum<T>::value> struct utest_type_deducer final {
...
template <typename T> UTEST_WEAK UTEST_OVERLOADABLE void utest_type_printer(const T t) {
...
static UTEST_INLINE char* utest_strncpy_gcc(char* const dst, const char* const src,
                                            const size_t size) {
#pragma GCC diagnostic push
...
#define UTEST_I_SETUP(FIXTURE)                                                                     \
    static void utest_i_setup_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture,          \
                                        size_t utest_index)

#define UTEST_I_TEARDOWN(FIXTURE)                                                                  \
    static void utest_i_teardown_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture,       \
                                           size_t utest_index)
...
int utest_main(int argc, const char* const argv[]) {
    ...
    for (index = 1; index < UTEST_CAST(size_t, argc); index++) {
        ...
        if (0 == UTEST_STRNCMP(argv[index], help_str, strlen(help_str))) {
            ...
            seed =
                UTEST_CAST(utest_uint32_t, strtoul(argv[index] + strlen(random_order_with_seed_str),
                                                   UTEST_NULL, 10));
            random_order = 1;
    ...
    printf("%s[==========]%s Running %" UTEST_PRIu64 " test cases.\n", colours[GREEN],
           colours[RESET], UTEST_CAST(utest_uint64_t, ran_tests));
    ...
    for (index = 0; index < utest_state.tests_length; index++) {
        ...
        if (UTEST_TEST_FAILURE == result) {
            ...
            failed_testcases =
                UTEST_PTR_CAST(size_t*, utest_realloc(UTEST_PTR_CAST(void*, failed_testcases),
            ...
            skipped_testcases =
                UTEST_PTR_CAST(size_t*, utest_realloc(UTEST_PTR_CAST(void*, skipped_testcases),
...

        {
            ...
            if (UTEST_TEST_FAILURE == result) {
                ...
                printf("%s[  SKIPPED ]%s %s (%" UTEST_PRId64 "%s)\n", colours[YELLOW],
                       colours[RESET], utest_state.tests[index].name, time, units[unit_index]);
            } else {
