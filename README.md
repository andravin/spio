...
State of the art convolutional neural networks do not have efficient GPU kernels.
Indeed, perhaps the only reason vision transformers appear to be a competitive model architecture
is because they have a more efficient implementation.

Spio aims to help improve the training performance of SOTA convnets by providing efficient kernels. The first kernel we implemented is grouped convolution, which we used in our
SOTA model.

In our [paper](https://arxiv.org/abs/2404.03617), we implemented grouped convolution for efficient inference. Spio implements grouped convolution for efficient training.

We will add kernels for additional layers to address more performance issues with SOTA convnets.
...
which would cause Python to find the local spio package instead of the installed package.
...
The Spio conv2d kernel tiles the input with rectangular blocks. This
reduces the overlap between tiles and minimizes global memory traffic.
